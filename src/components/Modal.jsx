...
const Modal = ({ isOpen, onClose, onSubmit, resetEmail, setResetEmail, resetError, successMessage }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 backdrop-blur-md">
      <motion.div
        className="bg-gray-900 p-5 rounded-lg shadow-lg max-w-sm w-full border border-blue-500 glow-effect"
      ...
      >
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            className="w-full bg-gradient-to-r from-blue-500 to-purple-600 text-white py-3 px-4 rounded-lg hover:bg-blue-600 shadow-lg transition-all glow-effect"
          >
            Send Reset Email
          </button>
        </form>
        ...
        {/* Close Modal Button */}
        <button
          className="bg-purple-600 text-white mt-4 py-2 px-4 rounded-lg hover:bg-purple-700 transition duration-300 glow-effect"
          onClick={onClose}
        >
          Close
        </button>
